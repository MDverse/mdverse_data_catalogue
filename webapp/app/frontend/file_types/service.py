...
def get_file_types_stats(session: Session) -> list:
    """
...
def get_list_of_files_for_a_file_type(session: Session, file_type: str) -> pd.DataFrame:
    """
    Returns a DataFrame with all files of a given file type.
    """
    # Create an alias for the parent file
    ParentFile = aliased(File)  # noqa: N806

    # Define a CASE expression:
    # if file is from a zip, then use parent's URL, else use file.url
    file_url_expr = case((File.is_from_zip_file, ParentFile.url), else_=File.url).label(
        "file_url"
    )
    ...
    statement = (
        ...
        # Left join the parent file so that files from a zip file
        # can retrieve the parent URL.
...
def get_gro_files_for_datatables(
    ...
    """
    Returns a list of GRO files with their related
    File, Dataset and Dataset.origin info.
    ...
    if dataset_id is not None:
        statement = statement.where(
            TopologyFile.file.has(File.dataset_id == dataset_id)
        )
...
def get_mdp_files_for_datatables(
    ...
    """
    Returns a list of MDP files with their related
    File, Dataset, and Dataset.origin info.
    ...
    if dataset_id is not None:
        statement = statement.where(
            ParameterFile.file.has(File.dataset_id == dataset_id)
        )
...
def get_xtc_files_for_datatables(
    ...
    """
    Returns a list of XTC files with their related
    File, Dataset, and Dataset.origin info.
    ...
    if dataset_id is not None:
        statement = statement.where(
            TrajectoryFile.file.has(File.dataset_id == dataset_id)
        )
