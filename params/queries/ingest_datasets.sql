...
WHERE data_source_label IS NOT NULL AND TRIM(data_source_label) != ''
GROUP BY data_source_label
ON CONFLICT (data_source_label) DO UPDATE SET
    url = COALESCE(data_sources.url, EXCLUDED.url),
...
WHERE database_label IS NOT NULL AND TRIM(database_label) != ''
ON CONFLICT (database_label) DO UPDATE SET
    url = EXCLUDED.url,
...
ON CONFLICT (full_name) DO UPDATE SET
    orcid = COALESCE(persons.orcid, EXCLUDED.orcid),
...
-- Insert unique papers.
INSERT INTO papers (doi, title, year, url, abstract, journal, keywords)
SELECT 
    doi,
    FIRST(title),
    FIRST(year),
...
ON CONFLICT (data_source_label, id_in_data_source) DO UPDATE SET
...
JOIN datasets d 
    ON d.id_in_data_source = sa.id_in_data_source 
   AND d.data_source_label = sa.data_source_label
JOIN persons p 
    ON p.full_name = sa.full_name
ON CONFLICT DO NOTHING;

-- Insert dataset-paper relationships using _stage_paper_links.
INSERT INTO datasets_papers_link (dataset_id, paper_id)
SELECT DISTINCT
    d.dataset_id,
    p.paper_id
FROM _stage_paper_links spl
JOIN datasets d 
    ON d.id_in_data_source = spl.id_in_data_source 
...
JOIN datasets d 
  ON d.data_source_label = sa.data_source_label 
 AND d.id_in_data_source = sa.id_in_data_source
ON CONFLICT (value, category_label, dataset_id, paper_id, file_id) DO UPDATE SET
...
SELECT DISTINCT
    CAST(m.molecule_id AS INTEGER) AS molecule_id,
...
JOIN molecules m 
  ON m.annotation_id = a.annotation_id
ON CONFLICT (molecule_id, database_label) DO NOTHING;