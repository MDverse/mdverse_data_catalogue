...
def get_or_create_models_with_one_attribute(
    ...
    """Get an instance of Model with a specific attribute value.
    ...
    -------
    SQLModel
        An instance of Model with the specified attribute value.
    """
...
def delete_files_for_update(
    engine: Engine, new_or_modified_datasets: list[int]
) -> None:
    ...
    with Session(engine) as session:
        ...
        # have a dataset_id. This is too complicated to base ourselves for deletion.
        # We will delete all the rows in these tables then add again the data.

        # WARNING: This is with the assumption that the parquet files used to ingest
        # the data will always have ALL the files,
        # not just new ones or a select amount of the old ones.
    ...
    logger.info(
        f"Total rows from FILES deleted from updated datasets: {result_files.rowcount}"
    )
    logger.info(
        f"Total rows from TRAJECTORY_FILES deleted: {result_trajectory.rowcount}"
    )
...
def load_datasets_data(parquet_path: str) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        A DataFrame containing the datasets data.
    """
    datasets_df = pd.read_parquet(parquet_path)
    ...
    datasets_df["author"] = (
        datasets_df["author"].str.replace(", ", ",").str.replace(";", ",")
    )
    datasets_df["keywords"] = (
        datasets_df["keywords"]
        .str.replace(", ", ",")
        .str.replace("; ", ";")
        .str.replace(",", ";")
    ...
    datasets_df["keywords"] = datasets_df["keywords"].str.lower()
    ...
    datasets_df["keywords"] = datasets_df["keywords"].apply(
        lambda x: x if pd.notna(x) else ""
    )
    ...
    datasets_df["data_source_url"] = datasets_df["data_source"].map(
        {
...
def load_files_data(parquet_path: str) -> pd.DataFrame:
    """Load files from parquet file.

    Returns
    -------
    pd.DataFrame
        A DataFrame containing the files data.
    """
...
def load_topology_data(parquet_path_topology: str) -> pd.DataFrame:
    """Load topology from Parquet file.
...
def load_parameter_data(parquet_path_parameters: str) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        A DataFrame containing the parameter files data.
    """
    ...
    parameter_df["integrator"] = parameter_df["integrator"].apply(
        lambda x: x if pd.notna(x) else "undefined"
    )
...
def load_trajectory_data(parquet_path_trajectory: str) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        A DataFrame containing the trajectory files data.
    """
    trajectory_df = pd.read_parquet(parquet_path_trajectory)

    return trajectory_df[
        ["dataset_origin", "dataset_id", "file_name", "atom_number", "frame_number"]
    ].rename(
        columns={
            "dataset_origin": "data_source",
            "dataset_id": "dataset_id_in_data_source",
...
def create_or_update_datasets_authors_origins_tables(
    ...
    with Session(engine) as session:
        for _, row in tqdm(
            datasets_df.iterrows(), total=total_rows, desc="Processing rows", unit="row"
        ):
            ...
            author_names = [
                name.strip() for name in row["author"].split(",") if name.strip()
            ]
            ...
            authors = [
                get_or_create_models_with_one_attribute(
                    session, Author, "name", name, dict_to_add_author
                )
    ...
    logger.info(
        "Entries ignored because they already exist in the database: "
        f"{len(datasets_ids_unchanged)}"
    )

    return datasets_ids_new + datasets_ids_modified
...
def create_files_tables(
    ...
    with Session(engine) as session:
        for _, row in tqdm(
            files_df.iterrows(), total=total_rows, desc="Processing rows", unit="row"
        ):
            ...
            if not existing_file:
                ...
                type_obj = get_or_create_models_with_one_attribute(
                    session, FileType, "name", file_type_name, dict_to_add_file_type
                )
                ...
                # If the file is from a zip file,
                # we need to find the parent file file_id
                if row["is_from_zip_file"] and parent_zip_file_name:
                    # Construct a key that combines the dataset id
                    # and parent's file name.
                    # Takes the dataset_id of the child file
                    # and the parent zip file_name.
                    key = (current_dataset.dataset_id, parent_zip_file_name)

                    # Option 1: Check if we have already found
                    # the parent file in the cache.
                    parent_zip_file_id = parent_files_by_name.get(key)
                    ...
                    if not parent_zip_file_id:
                        ...
                        # Option 2: Query the DB using both
                        # the file name and dataset id.
                        parent_statement = select(File)(
                        ...
                        if parent_obj:
                            ...
                            logger.error(
                                f"Parent file '{parent_zip_file_name}' "
                                f"not found for child '{row['name']}' "
                                f"with dataset_id {current_dataset.dataset_id}."
                            )
                ...
                session.refresh(
                    new_file_obj
                )  # Refresh because we need the file_id for the parent zip file
...
def create_simulation_tables(
    ...
    """Create TopologyFile, ParameterFile, and TrajectoryFile records in database."""
...
def data_ingestion():
    ...
    1) Populate the "datasets tables".
    ...
    new_or_modified_datasets = create_or_update_datasets_authors_origins_tables(
        datasets_df, engine
    )
